...
/**
 * Generic class for interacting with the Brandfolder API.
 *
 * @package Brandfolder
 */
class BrandfolderClient {
  ...
  /**
   * Brandfolder client constructor.
   *
   * @param string $api_key
   * @param \GuzzleHttp\ClientInterface|NULL $http_client
   */
  public function __construct($api_key, ClientInterface $http_client = NULL) {
   ...
   *
   * @todo: Return Brandfolder objects.
   */
   ...
   *
   * @todo: Return Collection objects.
   */
   ...
   * @param $result
   */
  public function restructureIncludedData(&$result) {
    $included = [];
...
//            });
   ...
   *
   * @todo: Consider always returning bool and requiring invokers to access the response_content property on success.
   *
  ...
  public function request($method, $path, $query_params = [], $body = NULL) {
    ...
    try {
      $this->response = $this->http_client->request($method, $this->endpoint . $path, $options);
    }
    ...
    $this->message = $this->response->getReasonPhrase();

    try {
      $this->response_content = \GuzzleHttp\json_decode($this->response->getBody()
        ->getContents());
    }
    catch (\RuntimeException | \Exception\InvalidArgumentException $e) {
      // @todo: Consider exception-specific handling.
      // @todo: Consider not overloading $this->code and instead using separate properties for HTTP response code and other codes.
      $this->status = $e->getCode();
      $this->message = $e->getMessage();
